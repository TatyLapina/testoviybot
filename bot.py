...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data in ["agree", "main_menu"]:
        ...
        await query.message.reply_text("✍️ Введи текст рассылки одним сообщением или /cancel для отмены:")
...
async def broadcast_send(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with get_conn() as conn:
        with conn.cursor() as cur:
            for idx, cid in enumerate(users, start=1):
                try:
                    ...
                    print(f"Ошибка при отправке {cid}: {e}")
                    failed += 1

                # каждые 5% показываем прогресс
                if idx % max(1, total // 20) == 0:
                    progress = round(idx / total * 100)
                    print(f"📤 Рассылка: {progress}% ({idx}/{total})")
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(button_handler, pattern="^(?!broadcast_start).*"))

    conv_handler = ConversationHandler(
        entry_points=[CallbackQueryHandler(button_handler, pattern="^broadcast_start$")],
        states={BROADCAST_WAITING: [MessageHandler(filters.TEXT & ~filters.COMMAND, broadcast_send)]},
